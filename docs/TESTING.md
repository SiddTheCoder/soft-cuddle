...
**Never mock the database for financial tests.** The constraints *are* the
...
test('a SaaS cannot approve a refund', async () => { /* insufficient scope */ })
...
test('an admin without TOTP cannot be created', async () => { /* constraint */ })
```
...
test('TDS on an agency payment lands in 1210', async () => { /* §9.7 */ })
test('P&L and balance sheet tie to the trial balance', async () => { /* to the paisa */ })
